...
try{
    ...
    foreach ($regularList as $listValue) {
       ...
       $mbHtmlBody = htmlspecialchars_decode($listValue["mb_html_body"], ENT_QUOTES);
        ...
        // PC画像リストの取得
        $pcRegularImageList = $AdmSupportMailOBJ->getSupportMailImageRegularData($listValue["id"], false);

        $pcImageData = array();
        $pcImageType = array();
        for ($i = 0; $i < count($pcRegularImageList); $i++) {
            if ($pcRegularImageList[$i]["file_name"]) {
                $pcImageData[$i] = file_get_contents(D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH . $pcRegularImageList[$i]["file_name"]);
                $size = getimagesize(D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH . $pcRegularImageList[$i]["file_name"]);
                $pcImageType[$i] = $size["mime"];
            }
        }

        // MB画像リストの取得
        $mbRegularImageList = $AdmSupportMailOBJ->getSupportMailImageRegularData($listValue["id"], true);

        $mbImageData = array();
        $mbImageType = array();
        for ($i = 0; $i < count($mbRegularImageList); $i++) {
            if ($mbRegularImageList[$i]["file_name"]) {
                $mbImageData[$i] = file_get_contents(D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH . $mbRegularImageList[$i]["file_name"]);
                $size = getimagesize(D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH . $mbRegularImageList[$i]["file_name"]);
        ...
        // メール画像ログの書き込み
        $AdmSupportMailOBJ->insertSupportMailImageLogByCron($mailMagaId, $pcRegularImageList, D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH, false);
        // メール画像ログの書き込み
        $AdmSupportMailOBJ->insertSupportMailImageLogByCron($mailMagaId, $mbRegularImageList, D_BASE_DIR . AdmSupportMail::SUPPORT_MAIL_REGULAR_IMAGE_PATH, true);
