...
class AffiliateControl extends ComCommon {
    ...
    public function insertData($insertArray) {
        ...
        if (!$dbResultOBJ = $this->insert("affiliate", $insertArray)) {
    ...
    public function updateData($updateArray, $whereArray = null) {
        ...
        if (!$dbResultOBJ = $this->update("affiliate", $updateArray, $whereArray)) {
    ...
    public function getAffiliateData($id) {
        ...
        $sql = $this->makeSelectQuery("affiliate", $columnArray, $whereArray);
    ...
    public function getAffiliateDataFromAdvcd($advCd, $sendType = "") {
        ...
        if (!$affiliateData) {
            ...
            // 自社ポータルは前方二文字がサイト識別コード
            $advKeyCd = substr($advCd, 0, 2);
...
*/
        ...
        } else if($aryAffiliateValue["from_first_payment"]){
            // 初入金パラメータを設定する
            if ($affiliateData["first_payment_parameter"]) {
                $url .= (strpos($url, "?") ? "&" . $affiliateData["first_payment_parameter"] : "?" . $affiliateData["first_payment_parameter"]);
                $setUserParam["affiliate_tag_first_payment_status"] = 1;
        ...
        if ($affiliateData["connect_type"] == self::CONNECT_TYPE_SOCKET) {
            ...
            if ($userId) {
                if($aryAffiliateValue["payment"]){
                    ...
                    //入金タグ関連のﾃﾞｰﾀをpayment_parameter_logにインサート
     ...
     * @return boolean
     */
    public function insertPaymentAffiliateTagLog($insertArray) {

        if (!is_array($insertArray)) {
            return false;
        }

        if (!$dbResultOBJ = $this->insert("payment_parameter_log", $insertArray)) {
            $this->_errorMsg[] = "データ登録できませんでした。";
            return false;
        }
     ...
     * @return mixed 入金発行タグ情報、失敗ならfalse
     */
    public function getPaymentTagLogData($userId) {
        ...
        $otherArray[] = "ORDER BY create_datetime DESC";

        $sql = $this->makeSelectQuery("payment_parameter_log", $columnArray, $whereArray,$otherArray);
