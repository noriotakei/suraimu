...
//get correct ids from search conditions
$is_have_correct_id = TRUE;

if ($param["mailmagazine_id"]) {
    $idArr = explode(",", $param["mailmagazine_id"]);
    ...
    if ($idArrNew) {
        $param["mailmagazine_id"] = implode(",", $idArrNew);
    }else {
        $is_have_correct_id = FALSE;
    }
}

if ($is_have_correct_id) {
    $dataList = $admMailMagazineOBJ->getMailReserveList($param, $offset, "send_datetime DESC", $dispCnt);
}
