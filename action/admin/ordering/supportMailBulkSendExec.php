...
if ($param["mail_reserve_type"] == AdmSupportMail::SUPPORTMAIL_TYPE_BULK) {
    ...
    while (list($key, $val) = each($orderingList)) {
        ...
        if (ComValidation::isMailAddress($sendData["pc_to_address"]) AND $userData["pc_address_status"] != $_config["define"]["ADDRESS_STATUS_DO"]) {
            ...
            $sendData["html_body"] = $pcHtmlBody;

            $sendData = $AdmSupportMailOBJ->convertMailElements($sendData, $orderingData["user_id"], $setArray);
            $pcSendMailData = $AdmSupportMailOBJ->smtpMailTo($sendData, $second, $pcImageData, $pcImageType);
            ...
            try{
                if (!$ComSendMagicDeliveryOBJ->sendMagicDelivery($pcSendMailData)) {
                    ...
                    $insertData["pc_html_body"] = $sendData["html_body"];
                }
        ...
        if (ComValidation::isMobileAddress($sendData["mb_to_address"]) AND $userData["mb_address_status"] != $_config["define"]["ADDRESS_STATUS_DO"]) {
            ...
            $sendData["html_body"] = $mbHtmlBody;
            $sendData = $AdmSupportMailOBJ->convertMailElements($sendData, $orderingData["user_id"], $setArray);
            $mbSendMailData = $AdmSupportMailOBJ->smtpMailTo($sendData, $second,$mbImageData, $mbImageType);
            ...
            try{
                if (!$ComSendMagicDeliveryOBJ->sendMagicDelivery($mbSendMailData)) {
                    ...
                    $insertData["mb_html_body"] = $sendData["html_body"];
                }
            } catch (Zend_Exception $e) {
                // デバッグメール
                $debugMail = "";
        ...
        // 配信ログの登録
        if ($insertData["pc_text_body"] OR $insertData["mb_text_body"] OR $insertData["pc_html_body"] OR $insertData["mb_html_body"]) {
    ...
    $supportMailSendLog["pc_html_body"] = $param["pc_html_body"];
    $supportMailSendLog["mb_subject"] = $param["mb_subject"];
    $supportMailSendLog["mb_text_body"] = $param["mb_text_body"];
    $supportMailSendLog["mb_html_body"] = $param["mb_html_body"];
   ...
   $mailMagaId = $AdmSupportMailOBJ->getInsertId();
    ...
    // メール画像ログの書き込み
    $AdmSupportMailOBJ->insertSupportMailImageLog($mailMagaId, "pc_image", false);
    // メール画像ログの書き込み
    $AdmSupportMailOBJ->insertSupportMailImageLog($mailMagaId, "mb_image", true);
...
// 予約サポートメール
    ...
    $supportMailLog["pc_html_body"] = $param["pc_html_body"];
    $supportMailLog["mb_subject"] = $param["mb_subject"];
    $supportMailLog["mb_text_body"] = $param["mb_text_body"];
    $supportMailLog["mb_html_body"] = $param["mb_html_body"];
   ...
   $mailMagaReserveId = $AdmSupportMailOBJ->getInsertId();

   // メール画像の追加
   if (!$AdmSupportMailOBJ->insertSupportMailImageReserve($mailMagaReserveId, "pc_image", false)) {
       $errSessOBJ->errMsg = $AdmSupportMailOBJ->getErrorMsg();
       header("Location: ./?action_ordering_SupportMailBulkInput=1" . $URLparam);
       exit;
   }
   ...
   // メール画像の追加
   if (!$AdmSupportMailOBJ->insertSupportMailImageReserve($mailMagaReserveId, "mb_image", true)) {
       $errSessOBJ->errMsg = $AdmSupportMailOBJ->getErrorMsg();
       header("Location: ./?action_ordering_SupportMailBulkInput=1" . $URLparam);
       exit;
   }
...
// 定期サポートメール
   ...
   // メール画像の追加
   if (!$AdmSupportMailOBJ->insertSupportMailImageRegular($mailMagaReserveId, "pc_image", false)) {
       $errSessOBJ->errMsg = $AdmSupportMailOBJ->getErrorMsg();
       header("Location: ./?action_ordering_SupportMailBulkInput=1" . $URLparam);
       exit;
   }
   ...
   // メール画像の追加
   if (!$AdmSupportMailOBJ->insertSupportMailImageRegular($mailMagaReserveId, "mb_image", true)) {
       $errSessOBJ->errMsg = $AdmSupportMailOBJ->getErrorMsg();
       header("Location: ./?action_ordering_SupportMailBulkInput=1" . $URLparam);
       exit;
   }
