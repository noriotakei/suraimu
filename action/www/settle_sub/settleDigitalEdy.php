...
if (!$orderingData = $OrderingOBJ->getOrderingDataFromAccessKey($param["odid"], $comUserData["user_id"])) {
    ...
    $insertOrderingArray["status"] = Ordering::ORDERING_STATUS_WAIT_DIGITALEDY;
    $insertOrderingArray["pay_type"] = Ordering::PAY_TYPE_DIGITALEDY;
...
// 注文情報があれば更新
    ...
    // 注文情報更新
    $updateOrderingArray["status"] = Ordering::ORDERING_STATUS_WAIT_DIGITALEDY;
    $updateOrderingArray["pay_type"] = Ordering::PAY_TYPE_DIGITALEDY;
...
//使用ブラウザのチェック
$isIeBrowser = preg_match('/ie/',$ComUtilityOBJ->getBrowser()) ? true : false;
$smartyOBJ->assign("isIeBrowser", $isIeBrowser);
