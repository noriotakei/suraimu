...
if (!$InfoStatusLogOBJ->isAccessed($infoStatusData["id"], $comUserData["user_id"])) {
    ...
    if ($infoStatusData["point"]) {
        ...
        if ($infoStatusData["point"] > $comUserData["point"]) {
            ...
            //強制的に商品の注文を追加
            $errMsgSessOBJ->errMsg[] = "御客様はﾎﾟｲﾝﾄ不足のため情報を閲覧できません｡";
            $errMsgSessOBJ->errMsg[] = "情報の閲覧を希望される場合は、ﾎﾟｲﾝﾄ追加をお願い致します｡";
...
// ↓既読↓
    ...
    if ($infoStatusData["redirect_url"]) {
        header("Location:." . htmlspecialchars_decode($infoStatusData["redirect_url"], ENT_QUOTES) . ($comURLparam ? "&" . $comURLparam : ""));
        exit;
    }
}
