...
if (!$_config["define"]["TEST_DEVELOPMENT_FLAG"]) {
    // コンビニダイレクト決済
    if (!$SettlementCvdOBJ->sendToCvd($orderingData, $comUserData, $param, false)) {
    ...
    // コンビニダイレクト決済
    if (!$SettlementCvdOBJ->testSendToCvd($orderingData, $comUserData, $param, false)) {
...
header("Location: ./?action_SettleCvdEnd=1&" . $URLparam . ($comURLparam ? "&" . $comURLparam : ""));
exit;
?>
