...
$SettlementCreditOBJ->setSettleType("mb");
...
$SettlementCreditOBJ->setHiddenData("usrmail", $comUserData["mb_address"] ? $comUserData["mb_address"] : $comUserData["pc_address"]);
$SettlementCreditOBJ->setHiddenData("siteurl", $_config["define"]["SITE_URL_MOBILE"] . "?action_Home=1" . ($comURLparam ? "&" . $comURLparam : "") . "&" . $sessId);

$creditUrl = $SettlementCreditOBJ->getCreditSettleUrl();
$creditHiddenTags = $SettlementCreditOBJ->makeCreditHiddenTags();

// クイックチャージ判定
$isQuick = $comUserData["credit_certify_phone_number_mb"] ? true : false;
...
// MB用、決済IP
$settleClientIp = SettlementCredit::CREDIT_CLIENT_IP_MB;
