...
foreach ((array)$itemList as $val) {
    ...
    $errSessOBJ->errMsg[] = "ご予約キャンセルできませんでした。";
        ...
        header("Location: ./?action_OrderingDelChk=1&" . $URLparam . ($comURLparam ? "&" . $comURLparam : "") . "&" . $sessId);
        exit;
    }
...
header("Location: ./?action_OrderingDelComplete=1" . ($comURLparam ? "&" . $comURLparam : "") . "&" . $sessId);
exit;
