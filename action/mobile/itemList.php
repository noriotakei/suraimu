...
$infoDispPositionExpList = $infoDispPositionOBJ->getInformationDisplayPositionList($itemDispPositionExpList, "mb_sort_seq DESC");
...
$infoDispPositionList = $infoDispPositionOBJ->getInformationDisplayPositionList($itemDispPositionList, "mb_sort_seq DESC");
...
foreach ($infoDispPositionList as $itemKey => $itemVal) {
    ...
    if ($infoItemList = $infoStatusOBJ->getInformationStatusList($itemVal["information_category_id"], $comUserData)) {
        foreach ($infoItemList as $itemListVal) {
            if($itemListVal[$convertInfoHtmlKey]){
                // 情報データコンバート用変換キーの取得
