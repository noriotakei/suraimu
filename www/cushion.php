...
if($dangerFlag == TRUE){
    if ($mbFlag) {
        ...
        $mailElements = $AutoMailOBJ->convertMailElements($mailElementsData["elements"]);
        // メール送信
...
/* -20101216-takuro PCからのMBアドレス登録の為コメントアウト
...
if($param["noRemail"] != TRUE){
    ...
    //登録リメール二通目がセットしてあったらそれも飛ばす。
    //処理が手抜き感満載ですがすぐに使わなくなると思うの…
    if($mailElementsData["elements"]["text_body_second"]){
    	$mailElementsData["elements"]["text_body"] = $mailElementsData["elements"]["text_body_second"] ;
    	$mailElementsData["elements"]["html_body"] = "" ;
    	$mailElements = $AutoMailOBJ->convertMailElements($mailElementsData["elements"], $userId, $convAry);
    	// メール送信
    	$AutoMailOBJ->mailTo($mailElements, "", $mailElementsData["image_data"], $mailElementsData["image_type"]);
    }
