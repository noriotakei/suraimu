...
<form action="./" method="post" enctype="multipart/form-data">
    <table border="0" cellspacing="0" cellpadding="0" class="TableSet01">
        ...
        <tr>
            <th>カテゴリーグループ</th>
            <td style="text-align: left;">
                {html_options name="category_group_id" options=$categoryGroupSelect selected=$registParam.item_category_group_id|default:1}</td>
            </td>
        </tr>
        <tr>
...
{if $itemCategoryList}
    ...
    <table border="0" cellspacing="0" cellpadding="0" id="list_table" class="TableSet04">
        ...
        {foreach from=$itemCategoryList item="val"}
        <tr>
            <td align="center"><a href="{make_link action="action_itemManagement_ItemCategoryData" getTags="icid="|cat:$val.id}">{$val.id}</a></td>
            <td align="left">{$val.name}</td>
            <td align="center">{$categoryGroupSelect[$val.item_category_group_id]}</td>
            <td align="center">{$val.sort_seq}</td>
            <td align="center">{$isDisplay[$val.is_display]}</td>
            <td>
                <form action="./" method="post" style="margin:2px 0px;">
                    <input type="hidden" name="icid" value="{$val.id}">
