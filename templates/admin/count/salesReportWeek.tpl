...
<div id="tabs">
    ...
    <div id="tabs-1">
        <table cellspacing="0" cellpadding="0" class="TableSet02" id="table" align="center">
            ...
            {foreach from=$dispDateArray key="key" item="val" name="loop"}
                {assign var="startWeekNum" value=$val.start_date|date_format:'%w'}
                {assign var="endWeekNum" value=$val.end_date|date_format:'%w'}
                {cycle values=", class=\"BgColor02\"" assign="style"}
                <tr {$style}>
                    <td rowspan="2">{$val.start_date|date_format:'%Y年%m月%d日'}({$weekArray[$startWeekNum]})<br>～{$val.end_date|date_format:'%Y年%m月%d日'}({$weekArray[$endWeekNum]})</td>
    ...
    <div id="tabs-graph">
        <table cellspacing="0" cellpadding="0" align="center">
            <tr>
            <td>{if $totalDataList.pay_total}<div class="jqPlot" id="barChart" style="height:300px; width:900px;">{/if}</div></td>
            </tr>
        </table>
    </div>
</div>
