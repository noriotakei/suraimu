        ...
        <form action="./" method="post" enctype="multipart/form-data">
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="TableSet01">
                ...
                {if $param.extension_type == IMAGETYPE_SWF || $param.extension_type == IMAGETYPE_SWC}
                    {html_image file="./img/thumbnails/swf.jpg" width="64" height="58" alt=$param.name}
                {else}
                    <img src="./{$bannerPath}{$param.file_name}.{$extensionTypeArray[$param.extension_type]}?{$smarty.now}" alt ="画像">
                {/if}
                </td>
                </tr>
                <tr>
                    <th>カテゴリー</th>
                    <td style="text-align: left;">{html_options name="banner_image_category_id" options=$categoryList selected=$param.banner_image_category_id}</td>
                </tr>
                <tr>
                <th>名前</th>
                <td style="text-align: left;">
                ...
                <tr>
                <th>PATH</th>
                    <td style="text-align: left;">{$config.define.SITE_URL}{$bannerPath}{$param.file_name}.{$extensionTypeArray[$param.extension_type]}</td>
                </tr>
                <tr>
                <th>FILE</th>
                <td style="text-align: left;">
