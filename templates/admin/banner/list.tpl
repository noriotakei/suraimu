...
<form action="./" method="post" enctype="multipart/form-data">
    <table border="0" cellspacing="0" cellpadding="0" class="TableSet01">
        <tr>
            <th>カテゴリー</th>
            <td style="text-align: left;">{html_options name="banner_image_category_id" options=$categoryList selected=$registParam.banner_image_category_id}</td>
        </tr>
        <tr>
        <th >名前</th>
        ...
        <tr>
            <th>ファイル名</th>
            <td style="text-align: left;"><input type="text" name="file_name" value="{$registParam.file_name}" size="50" style="ime-mode:disabled"></td>
        </tr>
        <tr>
        <th>FILE</th>
            <td style="text-align: left;">
...
{if $bannerList}
    ...
    {foreach from=$bannerList item="val"}
        <tr>
        <td><a href="{make_link action="action_banner_BannerUpd" getTags="banner_id="|cat:$val.id}">{$val.id}</a></td>
        <td>
        <div class="images">
            {if $val.extension_type == IMAGETYPE_SWF || $val.extension_type == IMAGETYPE_SWC}
                <a href="./{$bannerPath}{$val.file_name}.{$extensionTypeArray[$val.extension_type]}" toptions="type = flash, group = 'images', width = 550, height = 400, parameters = '{$smarty.now} =1', title={$val.name}">
                {html_image file="./img/thumbnails/swf.jpg" width="150" height="94" alt=$val.name}</a>
            {else}
                <a href="./{$bannerPath}{$val.file_name}.{$extensionTypeArray[$val.extension_type]}" toptions="type = image, group = 'images', parameters = '{$smarty.now} =1', title={$val.name}">
                {html_image file="./"|cat:$bannerPath|cat:$val.file_name|cat:"."|cat:$extensionTypeArray[$val.extension_type]|cat:"?"|cat:$smarty.now width="150" height="60" alt=$val.name}</a>
            {/if}
        </div>
        </td>
        <td>{$val.name}</td>
        <td>{$categoryList[$val.banner_image_category_id]}</td>
        <td>{$val.comment}</td>
        <td>
            {if $val.extension_type == IMAGETYPE_SWF || $val.extension_type == IMAGETYPE_SWC}
                <textarea class="selectText" readonly cols="50">{$config.define.SITE_URL}{$bannerPath}{$val.file_name}.{$extensionTypeArray[$val.extension_type]}</textarea></td>
            {else}
                <textarea class="selectText" readonly cols="50"><img src="{$config.define.SITE_URL}{$bannerPath}{$val.file_name}.{$extensionTypeArray[$val.extension_type]}"></textarea></td>
            {/if}
