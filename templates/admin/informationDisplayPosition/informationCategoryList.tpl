...
<h2 class="ContentTitle">情報フォルダ一覧</h2>
{* 更新時エラーコメント *}
{if $msg|@count}
...
<form action="./" method="post" enctype="multipart/form-data">
    <table border="0" cellspacing="0" cellpadding="0" class="TableSet01">
        ...
        <td colspan="2" style="text-align: center">
            <input type="submit" name="action_informationDisplayPosition_InformationCategoryExec" value="登　録" onClick="return confirm('登録しますか？')" />
        </td>
        </tr>
    </table>
</form>
...
{if $folderList}
    ...
    <form action="./" method="post" style="margin:2px 0px;" >
        <table border="0" cellspacing="0" cellpadding="0" id="list_table" class="TableSet04">
            ...
            {foreach from=$folderList item="val"}
            {cycle values="#CCCCCC," assign="tr_tag"}
            {if !$val.is_display}
                {assign var="tr_tag" value="#FF3333"}
            {/if}
            <tr bgcolor="{$tr_tag}">
                <td align="center"><a href="{make_link action="action_informationDisplayPosition_InformationDisplayPositionUpd" getTags="fid="|cat:$val.id}">{$val.id}</a></td>
                <td align="left">{$val.name}</td>
                <td align="center">{$val.sort_seq}</td>
                <td align="center">{$isDisplay[$val.is_display]}</td>
                <td style="text-align:center;">
                    <input type="checkbox" name="fid[]" value="{$val.id}">
                </td>
            </tr>
            {/foreach}
        </table>
        ...
        <input type="submit" name="action_informationDisplayPosition_InformationCategoryExec" value="削除" onClick="return confirm('削除しますか?')">
    </form>
