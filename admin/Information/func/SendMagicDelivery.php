...
require_once(D_BASE_DIR . "/func/SendMagic/SendMagicMail.php"); // SMTPメール送信処理ファイル
require_once(D_BASE_DIR . "/func/Utility.php");

class SendMagicDelivery extends SendMagicMail{
    ...
    const DEFAULT_SMTP_SEND_MAIL_IP = "103.18.100.163:25"; // SMTPデフォルトIP
    ...
    public function sendMagicDelivery($mailElemnts) {
        ...
        $UtilityOBJ = Utility::getInstance();
        $deviceCd = $UtilityOBJ->getDeviceFromMailAddress($mailElemnts['to']);
        if ($deviceCd == DEVICE_OTHER) {
            // PCはJIS
        ...
        if (empty($this->smtp_host) && empty($this->smtp_port)) {
            $mailServerIp = self::DEFAULT_SMTP_SEND_MAIL_IP;
            // IPセット
